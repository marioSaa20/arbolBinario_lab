...
void insertTreeMap(TreeMap * tree, void* key, void * value) {
    ...
    if(papa == NULL){
        tree->root = nuevo;
    } else if(tree->lower_than(key, papa->pair->key)){
        papa->left = nuevo;
    } else {
        papa->right = nuevo;
    }
...
void removeNode(TreeMap * tree, TreeNode* node) {
    ...
    void*auxSucesor = node->pair;
    node->pair = sucesor->pair;
    sucesor->pair = auxSucesor;
...
Pair * upperBound(TreeMap * tree, void* key) {
    ...
    while(aux != NULL){
        if(tree->lower_than(key, papa->pair->key)){
            ub_node = aux;
            aux = aux->left;
        } else {
            ub_node = aux;
        ...
        if(key <= aux->pair->key && aux < ub_node){
            return aux->pair;
        }
    }
    return NULL;
